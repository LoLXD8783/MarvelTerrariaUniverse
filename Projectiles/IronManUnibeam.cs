...
{
	public class IronManUnibeam : ModProjectile
	{
        public override string Texture => "MarvelTerrariaUniverse/Projectiles/IronManRepulsor";
        public int lifetime = 0; // Starting point for projectile lifetime calculation
        ...
        public override bool PreDraw(ref Color color)
        {
            ...
            Texture2D Texture = ModContent.Request<Texture2D>($"MarvelTerrariaUniverse/Projectiles/IronManRepulsor").Value;

            DrawLaser(Texture, Main.player[Projectile.owner].Center, Projectile.velocity, 10, Projectile.damage, -1.57f, 1f, 1000f, Color.White, (int)MOVE_DISTANCE);
            return false;
        }
        ...
        public void DrawLaser(Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
        {
            ...
            for (float i = transDist; i <= Distance; i += step)
            {
                ...
                Main.spriteBatch.Draw(texture, origin - Main.screenPosition,
                     new Rectangle(0, 26, 28, 21), i < transDist ? Color.Transparent : c, r,
        ...
        public override void AI()
        {
            ...
            Projectile.timeLeft = 2; // Refreshes projectile to keep alive for duration

            lifetime++;
            if (lifetime == 180) Projectile.Kill(); // Ends at determined lifetime
